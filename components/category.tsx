...
export function CategorySlider() {
      ...
      return (
        <section className="py-8 px-4 md:px-6">
          <div className="max-w-7xl mx-auto p-4 border border-red-300 bg-red-50 rounded-lg text-red-700 text-sm">
            <strong>CategorySlider error:</strong> {error}. Check the browser
            console for details.
          </div>
        </section>
      );
      ...
      return (
        <section className="py-8 px-4 md:px-6">
          <div className="max-w-7xl mx-auto p-4 border border-yellow-300 bg-yellow-50 rounded-lg text-yellow-700 text-sm">
            <strong>CategorySlider:</strong> API returned no active categories.
            Check the console log for the raw response and confirm the{" "}
            <code>status</code> field value matches "active" exactly.
          </div>
        </section>
      );
    }
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto">
        <h2 className="pb-10 text-xl md:text-3xl text-black font-bold mb-2 text-center">
          Featured Categories
        </h2>
        ...
        <div className="relative">
          ...
          >
            {categories.map((category) => (
              ...
              >
                <div className="relative w-[140px] aspect-square h-[140px] bg-muted rounded-lg overflow-hidden mb-3 group-hover:shadow-lg group-hover:bg-[#013E77] transition-shadow">
                  {category.image ? (
                    <Image
                ...
                <p className="text-sm text-black font-medium text-center  group-hover:text-[#013E77]">
                  {category.name}
                </p>
              </Link>
            ))}
          ...
          {categories.length > 6 && (
            <button
              ...
              className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-4 z-10 p-2 bg-black border border-border rounded-full shadow-md hover:bg-muted transition-colors"
