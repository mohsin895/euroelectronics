...
function TopSellingCard({ product }: { product: UiProduct }) {
  ...
  return (
    <div className="relative bg-white rounded-lg border border-border p-5 md:p-5 flex flex-col md:flex-row gap-5">
      {product.badge && (
        <span className="absolute -top-0 right-4 -translate-y-1/2 bg-red-600 text-black text-xs font-medium px-3 py-1.5 rounded-md flex items-center gap-1">
          🏷 {product.badge === "Sale" ? "Best Selling" : product.badge}
        </span>
      )}

      <Link href={`/product/${product.slug}`} className="shrink-0">
        <div className="relative text-black w-[250px] h-[300px] md:w-[250px] md:h-[300px]">
      ...
      <div className="flex flex-col justify-center gap-2 flex-1">
        <Link href={`/product/${product.slug}`}>
          <h3 className=" font-medium text-[#013E77] transition-colors">
            {product.name}
          </h3>
        </Link>

        <div className="flex items-center gap-2">
          <span className="text-lg font-semibold text-[#013E77]">
        ...
        <div className="flex items-center gap-3 mt-1">
          <button
            ...
            disabled={isAdding}
            className="flex items-center gap-1.5 border border-[#013E77] text-[#013E77] text-sm font-medium px-3 py-1.5 rounded-md hover:bg-orange-50 disabled:opacity-50 transition-colors"
          >
          ...
          <button
            ...
            disabled={isBuying}
            className="flex items-center gap-1.5 bg-[#013E77] text-white text-sm font-medium px-3 py-1.5 rounded-md hover:bg-[#013E77] disabled:opacity-50 transition-colors"
          >
