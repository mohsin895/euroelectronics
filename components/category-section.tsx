...
export function CategoryProduct({ categories, products, categoryName, brands = [] }: Props) {
    // ── filter/sort state ──────────────────────────────────────────────────
    ...
    const [selectedBrand, setSelectedBrand] = useState<number | null>(null)  // ← number, not string
    ...
    const filtered = useMemo(() => {
        ...
        if (selectedBrand) {
            list = list.filter(p => p.brand_id === selectedBrand)  // ← use brand_id
        }
        list = list.filter(p => {
        ...
        return list
    }, [products, selectedCategory,selectedBrand, priceMin, priceMax, sort])
        ...
        <div className="flex flex-col gap-4">
                    ...
                    {catExpanded && (
                        <ul className="p-2">
                            ...
                            {categories.map(cat => (
                                <li key={cat.id}>
                                    <button
                                        onClick={() => router.push(`/subcategory/${cat.slug}`)}  // ← navigate instead of filter
                                        className={`group flex w-full cursor-pointer items-center justify-between rounded-lg px-3 py-2.5 text-left text-sm transition-colors
            ...
            <span className="flex items-center gap-2">
                ...
                Brands
                    ...
                    {brandExpanded && (
                        <ul className="p-2 max-h-64 overflow-y-auto" style={{ scrollbarWidth: "thin" }}>
                            <li>
                                <button
