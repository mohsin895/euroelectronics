...
import { fetchProductsBySubCategory, type UiProduct } from '@/lib/products'
...
export function SubCategoryPageClient({ slug }: { slug: string }) {
    ...
    useEffect(() => {
        ...
        fetchProductsBySubCategory(slug)
            ...
            .catch((err) => {
                console.error('[SubCategoryPage] Failed to load sub-category:', err)
                setError(
                    err instanceof Error && err.message === 'NOT_FOUND'
                        ? 'Sub-category not found.'
                        : 'Unable to load this sub-category right now.'
                )
            })
    ...
    const filteredProducts = useMemo(() => {
        ...
        return list
    }, [products, selectedBrands, newArrivalOnly, minPrice, maxPrice, sortBy])

    // reset to page 1 whenever filters/sort change
    useEffect(() => {
        setPage(1)
    }, [selectedBrands, newArrivalOnly, minPrice, maxPrice, sortBy])
